...
describe("normalizeAdminPostQuery — whole-record fail-closed normalization", () => {
  it("defaults to page 1 / ALL with the fixed page size", () => {
  ...
  it("never lets pageSize come from the URL", () => {
    const result = normalizeAdminPostQuery({ page: "2" });
    expect(result.pageSize).toBe(ADMIN_POST_PAGE_SIZE);
  });
});

describe("toAdminPostTransportQuery", () => {
  it("produces the full frozen query shape with search and sort fixed", () => {
    expect(toAdminPostTransportQuery({ page: 2, status: "DRAFT", pageSize: 20 })).toEqual({
      page: 2,
      pageSize: 20,
      status: "DRAFT",
      search: "",
