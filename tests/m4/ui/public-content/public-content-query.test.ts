...
describe("normalizePublicContentAdminQuery", () => {
  ...
  it("collapses to canonical when unknown keys are present", () => {
    expect(
      normalizePublicContentAdminQuery({ unknownKey: "value" } as unknown as Record<string, string | string[] | undefined>, RESOURCE),
    ).toEqual({
      resource: RESOURCE,
      page: 1,
      visibility: "ALL",
  ...
  it("collapses to canonical when array values are present", () => {
    expect(
      normalizePublicContentAdminQuery({ page: ["2", "3"] } as Record<string, string | string[] | undefined>, RESOURCE),
    ).toEqual({
      resource: RESOURCE,
      page: 1,
      visibility: "ALL",
