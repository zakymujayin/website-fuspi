...
test.describe("M3 public Berita experience", () => {
  test.skip(!DATABASE_URL, "Public Berita browser tests require an isolated PostgreSQL database.");
  ...
  test.beforeAll(async () => {
    // Pre-clean any leftover data from interrupted runs
    await pool.query(`DELETE FROM "PostTranslation" WHERE "postId" IN (SELECT "id" FROM "Post" WHERE "slug" LIKE 'e2e-br-%')`);
    await pool.query(`DELETE FROM "PostTag" WHERE "postId" IN (SELECT "id" FROM "Post" WHERE "slug" LIKE 'e2e-br-%')`);
    await pool.query(`DELETE FROM "Post" WHERE "slug" LIKE 'e2e-br-%'`);
    await pool.query(`DELETE FROM "CategoryTranslation" WHERE "categoryId" IN (SELECT "id" FROM "Category" WHERE "slug" LIKE 'e2e-br-%')`);
    await pool.query(`DELETE FROM "Category" WHERE "slug" LIKE 'e2e-br-%'`);
    await pool.query(`DELETE FROM "Media" WHERE "originalName" LIKE 'e2e-br-%'`);
    await pool.query(`DELETE FROM "User" WHERE "email" LIKE '%example.invalid'`);
    ...
    // PUBLISHED BERITA with ID + cover
    ...
    await pool.query(
      ...
      [randomUUID(), pubId1, title1, "Kutipan berita publikasi pertama.", HOSTILE_HTML, "Meta Berita Pertama", "Deskripsi meta berita pertama.", "Keterangan gambar berita pertama."],
    );
    ...
    // PUBLISHED BERITA with ID only (fallback)
    ...
    await pool.query(
      `INSERT INTO "PostTranslation" ("id", "postId", "locale", "title", "excerpt", "content", "metaDesc", "status")
       VALUES ($1, $2, 'id', $3, 'Kutipan berita hanya ID.', '<p>Konten hanya dalam bahasa Indonesia.</p>', 'Deskripsi hanya ID.', 'PUBLISHED')`,
      [randomUUID(), pubIdID, titleIDOnly],
    );
    ...
    // Pagination posts (11 extra with OLDER dates to go to page 2)
    for (let i = 2; i <= 12; i++) {
      ...
      await pool.query(
        `INSERT INTO "Post" ("id", "type", "slug", "status", "publishedAt", "authorId", "updatedAt")
         VALUES ($1, 'BERITA', $2, 'PUBLISHED', $3, $4, NOW())`,
        [pid, s, new Date(Date.UTC(2024, 0, i)), authorId],
      );
      await pool.query(
        `INSERT INTO "PostTranslation" ("id", "postId", "locale", "title", "content", "status")
  ...
  test("ID: list pagination shows page 2", async ({ page }) => {
    await page.goto("/id/berita?page=2");
    await expect(page.getByText("Berita Halaman 2")).toBeVisible();
    const pagination = page.locator("nav[aria-label='Berita']").last();
    await expect(pagination).toBeVisible();
    await expect(pagination.getByText("2")).toBeVisible();
  });

  test("ID: hostile page values normalize safely without reflection", async ({ page }) => {
    ...
    // Should clamp to last valid page without reflecting 99999
  ...
  test("ID: list card metadata without fabricated view count/tags/search/archives", async ({ page }) => {
    ...
    // Category slug is humanized from the marker-prefixed slug
  ...
  test("ID: detail renders safe sanitized HTML — no script/event/js-url escape", async ({ page }) => {
  ...
  test("ID: detail shows Jakarta date and reading minutes, no fabricated stats", async ({ page }) => {
    await page.goto(`/id/berita/${slug1}`);
    await expect(page.getByText(/(?:15 Juli 2025|16 Juli 2025)/).first()).toBeVisible();
  ...
  test("ID: unknown slug → not-found content", async ({ page }) => {
  ...
  test("ID: wrong-type slug → not-found", async ({ page }) => {
  ...
  test("ID: detail has canonical, hreflang, OG metadata — no secrets", async ({ page }) => {
    ...
    await expect(canonical).toHaveAttribute("href", new RegExp(slugM));
  ...
  test("ID: breadcrumb with semantic links", async ({ page }) => {
    await page.goto(`/id/berita/${slugM}`);
    const bc = page.locator("nav[aria-label*='Berita' i]");
    await expect(bc).toBeVisible();
  });

  // ── EN Locale ──

  test("EN: list shows EN translation or ID fallback with lang=id dir=ltr", async ({ page }) => {
    await page.goto("/en/berita");
    await expect(page.getByText(titleMultiEN)).toBeVisible();
    // The ID-only post may appear on the first page
    const idOnly = page.getByText(titleIDOnly);
    if (await idOnly.count() > 0) {
      await expect(idOnly.first()).toBeVisible();
      await expect(page.getByRole("status")).toBeVisible();
    }
  });

  test("EN: ID-only detail shows lang=id dir=ltr on H1", async ({ page }) => {
  ...
  test("AR: ID fallback has lang=id dir=ltr inside RTL document", async ({ page }) => {
  ...
  test("AR: breadcrumb fallback title crumb has its own lang=id", async ({ page }) => {
    await page.goto(`/ar/berita/${slugID}`);
    const bc = page.locator("nav[aria-label*='Berita' i]").first();
    await expect(bc).toBeVisible();
    const idCrumb = bc.locator("[lang='id']");
    await expect(idCrumb).toBeVisible();
  });

  // ── Accessibility ──

  test("ID: axe WCAG AA on detail page", async ({ page }) => {
    await page.goto(`/id/berita/${slug1}`);
  ...
  test("ID: axe WCAG AA on list page", async ({ page }) => {
    await page.goto("/id/berita");
    const results = await new AxeBuilder({ page })
      .exclude("footer")
      .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"])
  ...
  test("AR: axe WCAG AA on Arabic detail", async ({ page }) => {
    await page.goto(`/ar/berita/${slugM}`);
    const results = await new AxeBuilder({ page })
      .exclude("footer")
      .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"])
  ...
  test("ID: no horizontal overflow on list at all breakpoints", async ({ page }) => {
    ...
    for (const width of BREAKPOINTS) {
      ...
      await page.waitForTimeout(200);
      const overflow = await horizontalOverflow(page);
      expect(overflow, `list overflow at ${width}px`).toBeLessThanOrEqual(0);
    }
  });

  test("AR: no horizontal overflow on detail in RTL", async ({ page }) => {
    await page.goto(`/ar/berita/${slug1}`);
    for (const width of BREAKPOINTS) {
      await page.setViewportSize({ width, height: 900 });
      await page.waitForTimeout(200);
      const overflow = await horizontalOverflow(page);
      expect(overflow, `AR overflow at ${width}px`).toBeLessThanOrEqual(0);
    }
  });
  ...
  test("AR: keyboard focus visible on detail links", async ({ page }) => {
    ...
    const focused = page.locator("nav a:focus");
