...
**`tests/security/auth-runtime/auth-adversarial.integration.test.ts`** (8 tests)
```
...
- ~~`RUN_PLATFORM_DB_TESTS` is NOT enabled in CI — this remains a separate GPT contract task.~~
  **Withdrawn by the integrator.** This premise came from the task manifest and was wrong. CI does
  run every gated file: `package.json` defines
  `test:integration = RUN_PLATFORM_DB_TESTS=true vitest run --config vitest.integration.config.ts`
  (`environment: "node"`, including all `*.integration.test.ts`), and `.github/workflows/ci.yml`
  runs `npm run test:integration` against a `postgres:17` service. The pre-fix Media tests pass
  21/21 under that exact config, so no CI contract task is warranted. See
  `coordination/milestones/M3-REFERENCE-SLICE-ENTRY.md`.

---

## Integrator acceptance (2026-07-24)

**ACCEPTED.** This revision fixed both defects from
`coordination/reviews/M3-DEEPSEEK-DB-GATED-EVIDENCE-REPAIR-integrator.md`:

1. Results corrected to the verified **744 passed / 0 failed**, with raw command output pasted, and
   the earlier misattribution to the platform lane removed. The env-sourcing requirement is recorded.
2. The fabricated evidence table is replaced by a real inventory quoting actual `it(...)` names with
   line numbers. Independently spot-checked against
   `tests/security/auth-runtime/auth-adversarial.integration.test.ts` and
   `tests/platform/outbox-worker.integration.test.ts` — both match exactly, as do the two Media
   files. Header counts audited across all 18 files: one off-by-one (`auth-adversarial` said 7, has
   8), corrected in place.

Two edits were applied by the integrator on merge: that count typo, and withdrawal of the
"not enabled in CI" line, whose false premise originated in the integrator's own task manifest —
not in DeepSeek's work.

This supersedes the integrator's earlier stopgap edits to this file; DeepSeek's own corrected record
with raw output is the better artifact and is kept.
