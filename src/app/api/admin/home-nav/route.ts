...
import {
  executeHomeNavCommand,
  getHomeNavDetail,
  homeNavHttpStatus,
  listHomeNav,
  normalizeHomeNavSearchParams,
} from "@/features/home-nav/domain";
...
export async function GET(request: Request) {
  ...
  const detailResource = url.searchParams.get("resource");

  if (detailId && detailResource) {
    const session = await getRequestSession();
    const result = await getHomeNavDetail(
      getPrismaClient(),
      session.ok ? session.session : null,
      {resource: detailResource, id: detailId},
    );
    return json(result.ok ? result.data : result, homeNavHttpStatus(result));
  }

  const query = normalizeHomeNavSearchParams(url.searchParams);
  if (!query.ok) return json(query, homeNavHttpStatus(query));
  const session = await getRequestSession();
  const result = await listHomeNav(
    getPrismaClient(),
    session.ok ? session.session : null,
    query.data,
  );
...
export async function POST(request: Request) {
  if (!isSameOriginRequest(request.headers)) {
    ...
    return json(result, homeNavHttpStatus(result));
  }
  ...
  const session = await getRequestSession();
  const result = await executeHomeNavCommand(
    getPrismaClient(),
    session.ok ? session.session : null,
    body.data,
  );
  if (result.ok) {
    for (const locale of ["id", "en", "ar"] as const) {
      revalidatePath(`/${locale}/admin/home-nav`);
      if (result.resource === "MENU_ITEM" || result.resource === "SITE_SETTING") {
