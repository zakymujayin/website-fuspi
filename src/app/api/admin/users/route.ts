...
import {AdminUserMutationResultSchema, normalizeAdminUserSearchParams} from "@/contracts/admin-foundation";
import {adminFoundationHttpStatus, executeAdminUserCommand, listAdminUsers} from "@/features/admin/foundation";
...
function failure(code: "CSRF_INVALID" | "REQUEST_INVALID") {
  return AdminUserMutationResultSchema.parse({ok: false, code});
}

export async function GET(request: Request) {
  let query: ReturnType<typeof normalizeAdminUserSearchParams>;
  try {
    query = normalizeAdminUserSearchParams(new URL(request.url).searchParams);
  } catch {
    const result = failure("REQUEST_INVALID");
    return json(result, adminFoundationHttpStatus(result));
  }
  const session = await getRequestSession();
  const result = await listAdminUsers(getPrismaClient(), session.ok ? session.session : null, query);
...
export async function POST(request: Request) {
  ...
  const session = await getRequestSession();
  const result = await executeAdminUserCommand(getPrismaClient(), session.ok ? session.session : null, body.data);
  if (result.ok) {
    for (const locale of ["id", "en", "ar"] as const) revalidatePath(`/${locale}/admin/pengguna`);
  }
  return json(result, adminFoundationHttpStatus(result));
}
