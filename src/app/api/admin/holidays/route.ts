...
import {
  executeHolidayCommand,
  holidayHttpStatus,
  listHolidays,
  normalizeHolidaySearchParams,
} from "@/features/holiday/domain";
...
export async function GET(request: Request) {
  const query = normalizeHolidaySearchParams(new URL(request.url).searchParams);
  if (!query.ok) return json(query, holidayHttpStatus(query));
  ...
  return json(result.ok ? result.data : result, holidayHttpStatus(result));
}

export async function POST(request: Request) {
  if (!isSameOriginRequest(request.headers)) {
    const result = {ok: false as const, code: "CSRF_INVALID" as const};
    return json(result, holidayHttpStatus(result));
  }
  ...
  const session = await getRequestSession();
  const result = await executeHolidayCommand(
    getPrismaClient(),
    session.ok ? session.session : null,
    body.data,
  );
  if (result.ok) {
    for (const locale of ["id", "en", "ar"] as const) {
      revalidatePath(`/${locale}/admin/holidays`);
      revalidatePath(`/${locale}/tickets`);
      revalidatePath(`/${locale}/tiket`, "page");
      revalidatePath(`/${locale}/layanan/sla`, "page");
      revalidatePath(`/${locale}/layanan/tiket`, "page");
    }
  }
  return json(result, holidayHttpStatus(result));
}
