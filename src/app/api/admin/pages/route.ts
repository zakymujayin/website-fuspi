...
import {AdminPageMutationResponseSchema} from "@/contracts/page-admin";
import {
  adminPageHttpStatus,
...
export async function GET(request: Request) {
  const normalized = normalizeAdminPageSearchParams(new URL(request.url).searchParams);
  if (!normalized.ok) return json(normalized, adminPageHttpStatus(normalized));
...
export async function POST(request: Request) {
  if (!isSameOriginRequest(request.headers)) {
    ...
    return json(result, adminPageHttpStatus(result));
  }
  ...
  const session = await getRequestSession();
  const result = await executeAdminPageCommand(
    getPrismaClient(),
    session.ok ? session.session : null,
    body.data,
  );
  ...
  if (result.ok) {
    for (const locale of ["id", "en", "ar"] as const) {
      ...
      revalidatePath(`/${locale}/admin/pages/[pageId]/edit`, "page");
      revalidatePath(`/${locale}/halaman/[slug]`, "page");
    }
  }
