...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {title: t("columns")};
}

export default async function ColumnsPage({params}: {params: Promise<{locale: AppLocale}>}) {
  const {locale} = await params;
  ...
  return (
    <Container className="py-12 md:py-20">
      <nav aria-label="Breadcrumb" className="mb-8 flex items-center gap-2 text-sm text-slate-500">
        ...
        <span className="text-slate-800">{tNav("columns")}</span>
      </nav>
      <SectionHeading as="h1" title={t("columnsTitle")} />
      {rows.length > 0 ? (
