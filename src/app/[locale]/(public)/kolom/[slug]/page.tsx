...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (load.kind === "unavailable" || !load.result.ok) {
    return { title: t("kolom.title"), description: t("kolom.metaDescription") };
  }
  ...
  const alternates = buildLocaleAlternates(`${COLUMNS_PATH}/${slug}`, locale, siteOrigin);
...
async function fetchSidebarItems(
  ...
  try {
    ...
    const result = await listPublicPosts(
      database,
      { locale, type: "KOLOM", page: 1, pageSize: SIDEBAR_FETCH_SIZE },
      process.env.UPLOAD_PUBLIC_URL ?? "",
    );
    if (!result.ok) return [];
...
export default async function ColumnDetailPage({ params }: PageProps) {
  ...
  const siteOrigin = validateSiteOrigin(process.env.NEXT_PUBLIC_SITE_URL);
  const detailUrl = buildLocaleAlternates(`${COLUMNS_PATH}/${slug}`, locale, siteOrigin).canonical;
  const homeUrl = buildLocaleAlternates("/", locale, siteOrigin).canonical;
  const listUrl = buildLocaleAlternates(COLUMNS_PATH, locale, siteOrigin).canonical;
  const cover = resolveCoverImageSrc(post.cover, siteOrigin);
  const absoluteCoverUrl = cover.kind === "image" && siteOrigin ? new URL(cover.src, siteOrigin).toString() : null;

  const breadcrumbItems = [
    { label: t("breadcrumbHome"), href: "/" },
    { label: t("kolom.breadcrumbLabel"), href: COLUMNS_PATH },
  ...
  return (
    <Container className="py-12 md:py-20">
        ...
        <>
          <PostJsonLd
            data={buildBreadcrumbJsonLd([
              ...
              { name: t("kolom.breadcrumbLabel"), url: listUrl },
      ...
      <PostBreadcrumb items={breadcrumbItems} ariaLabel={t("kolom.breadcrumbLabel")} />
