...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (load.kind === "unavailable" || !load.result.ok) {
    return { title: t("news.title"), description: t("news.metaDescription") };
  }
...
async function fetchSidebarItems(
  ...
  try {
    ...
    const result = await listPublicPosts(
      database,
      { locale, type: "BERITA", page: 1, pageSize: SIDEBAR_FETCH_SIZE },
      process.env.UPLOAD_PUBLIC_URL ?? "",
    );
    if (!result.ok) return [];
        ...
        <>
          <PostJsonLd
            data={buildBreadcrumbJsonLd([
              ...
              { name: t("news.breadcrumbLabel"), url: newsUrl },
