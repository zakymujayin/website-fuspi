...
import { LecturerAdminWorkspace } from "@/components/admin/lecturer/lecturer-admin-workspace";
import { LecturerRelationsManager } from "@/components/admin/lecturer/lecturer-relations-manager";
import {LecturerAcademicRecordsManager} from "@/components/admin/lecturer/lecturer-academic-records-manager";
...
import {loadAdminLecturerAcademicRecords} from "@/features/academic/lecturer-academic-records";
...
export default async function EditLecturerPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const academicRecords = await loadAdminLecturerAcademicRecords(getPrismaClient(), session.session, id);
  if (!academicRecords.ok) notFound();
  ...
  return <section aria-labelledby="admin-lecturer-edit-title" className="mx-auto flex w-full max-w-[1180px] flex-col"><div><Link href="/admin/dosen" className="text-sm font-medium text-primary hover:underline">← {locale === "id" ? "Kembali ke daftar" : locale === "en" ? "Back to list" : "العودة إلى القائمة"}</Link><h1 id="admin-lecturer-edit-title" className="mt-4 font-display text-3xl tracking-tight text-slate-950">{locale === "id" ? "Sunting dosen" : locale === "en" ? "Edit lecturer" : "تحرير المحاضر"}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{locale === "id" ? "Perbarui data dosen, penempatan program studi, dan rekam akademiknya." : locale === "en" ? "Update the lecturer data, study program assignment, and academic record." : "حدّث بيانات المحاضر وتعيين البرنامج والسجل الأكاديمي."}</p></div><LecturerAdminWorkspace locale={locale as AppLocale} profile={<LecturerEditorForm locale={locale as "id" | "en" | "ar"} mode="edit" initialDraft={draft} programs={programs} />} records={<LecturerRelationsManager locale={locale as AppLocale} lecturerId={id} relations={relations.data} />} academic={<LecturerAcademicRecordsManager locale={locale as AppLocale} lecturerId={id} records={academicRecords.data} />} /></section>;
}
