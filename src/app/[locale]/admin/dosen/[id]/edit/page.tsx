...
export default async function EditLecturerPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <section aria-labelledby="admin-lecturer-edit-title" className="flex flex-col gap-5"><div><Link href="/admin/dosen" className="text-sm font-medium text-primary hover:underline">← {locale === "id" ? "Kembali ke daftar" : locale === "en" ? "Back to list" : "العودة إلى القائمة"}</Link><h1 id="admin-lecturer-edit-title" className="mt-4 font-display text-3xl tracking-tight text-slate-950">{locale === "id" ? "Sunting dosen" : locale === "en" ? "Edit lecturer" : "تحرير المحاضر"}</h1><p className="mt-2 text-sm text-slate-500">{locale === "id" ? "Perbarui data dosen dan penempatan program studinya." : locale === "en" ? "Update the lecturer data and study program assignment." : "حدّث بيانات المحاضر وتعيين البرنامج الدراسي."}</p></div><LecturerEditorForm locale={locale as "id" | "en" | "ar"} mode="edit" initialDraft={draft} programs={programs} /></section>;
}
