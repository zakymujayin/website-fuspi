...
const RESOURCE: PublicContentResource = "EVENT";
const ADMIN_PATH = "agenda";
...
export default async function AdminPublicContentPage({ params, searchParams }: Props) {
  ...
  const decision = decideProtectedRoute(session, appLocale, `/${appLocale}/admin/agenda`);
    ...
    <section aria-labelledby="admin-public-content-title" className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-prose text-sm text-slate-500">{t("EVENT.description")}</p>
        </div>
        <Link
        ...
        <>
          <div role="tablist" aria-label={t("filterAriaLabel")} className="flex flex-wrap gap-1">
            {visibilityOptions.map((v) => {
              ...
              const href = `/admin/agenda?${params.toString()}`;
              return isActive ? (
