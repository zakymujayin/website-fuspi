...
const RESOURCE: PublicContentResource = "FAQ";
const ADMIN_PATH = "faq";
...
export default async function AdminPublicContentPage({ params, searchParams }: Props) {
  ...
  const decision = decideProtectedRoute(session, appLocale, `/${appLocale}/admin/faq`);
    ...
    <section aria-labelledby="admin-public-content-title" className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-prose text-sm text-slate-500">{t("FAQ.description")}</p>
        </div>
        <Link
          href={`/admin/faq/new`}
        ...
        <>
          <div role="tablist" aria-label={t("filterAriaLabel")} className="flex flex-wrap gap-1">
            {visibilityOptions.map((v) => {
              ...
              const href = `/admin/faq?${params.toString()}`;
              return isActive ? (
