...
export default async function EditFaqPage({ params }: Props) {
  ...
  const decision = decideProtectedRoute(session, appLocale, `/${appLocale}/admin/faq/${id}/edit`);
  if (!decision.allow) redirect(decision.redirectTo);

  const result = await getPublicContentAdminDetailAction({ resource: "FAQ", id });
  if (!result.ok) notFound();

  const t = await getTranslations("AdminPublicContent");

  return (
    <section aria-labelledby="admin-faq-edit-title" className="flex flex-col gap-6">
      <div>
        <h1 id="admin-faq-edit-title" className="section-rule font-display text-2xl text-slate-900">
          {t("FAQ.title")}
        </h1>
        <p className="mt-2 max-w-prose text-sm text-slate-500">{t("FAQ.editDescription")}</p>
      </div>
