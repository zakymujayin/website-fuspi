...
const RESOURCE: PublicContentResource = "PARTNERSHIP";
const ADMIN_PATH = "kerjasama";
...
export default async function AdminPublicContentPage({ params, searchParams }: Props) {
  ...
  const decision = decideProtectedRoute(session, appLocale, `/${appLocale}/admin/kerjasama`);
    ...
    <section aria-labelledby="admin-public-content-title" className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-prose text-sm text-slate-500">{t("PARTNERSHIP.description")}</p>
        </div>
        <Link
          href={`/admin/kerjasama/new`}
        ...
        <>
          <div role="tablist" aria-label={t("filterAriaLabel")} className="flex flex-wrap gap-1">
            {visibilityOptions.map((v) => {
              ...
              const href = `/admin/kerjasama?${params.toString()}`;
              return isActive ? (
                ...
                <li key={item.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4 sm:px-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        href={`/admin/kerjasama/${item.id}/edit`}
