...
/**
 * List view for Post type=KOLOM ("Gagasan dari Sivitas Akademika"). Reuses the same list/filter/
 * pagination components as /admin/posts — only the `type` passed to the transport query and the
 * link base paths differ. `listAdminPosts` already scopes by `type` generically, so this list is
 * fully functional today; only Create/Edit are blocked pending the admin transport contract update
 * (see the task handoff).
 */
