...
export function VideoSection({
  ...
  return (
    <section className={`${styles.section} bg-slate-100`}>
      <Container>
        <HomeSectionHeading title={title} eyebrow={eyebrow} description={subtitle} />
        <div className={`grid items-start gap-6 ${featured && videos.length ? "lg:grid-cols-12" : ""}`}>
            ...
            {videos.length > 0 ? (
              <div className={featured ? "grid gap-5 lg:col-span-4" : `grid gap-6 ${videos.length === 3 ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
