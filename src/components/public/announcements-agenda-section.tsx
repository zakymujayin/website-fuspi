...
export async function AnnouncementsAgendaSection({ locale, announcements, events }: AnnouncementsAgendaSectionProps) {
  ...
  return (
    <section className="border-t border-slate-200 bg-gradient-to-br from-royal-50 to-royal-100/50 py-12 md:py-16">
      <Container>
        <div className="grid gap-12 lg:grid-cols-2">
          {announcements.length > 0 && (
            <div>
              <div className="mb-6 flex items-center justify-between">
                <h2 className="section-rule font-display text-xl font-bold text-slate-900 md:text-2xl">{t("announcementsTitle")}</h2>
                <Link
                  href="/pengumuman"
                ...
                {announcements.map((item, index) => (
                  <Reveal key={item.id} index={index}>
                    <Link
                      ...
                      className="group flex w-full flex-1 items-center gap-4 rounded-xl border border-slate-100 bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                    >
          ...
          {events.length > 0 && (
            <div>
              <div className="mb-6 flex items-center justify-between">
                <h2 className="section-rule font-display text-xl font-bold text-slate-900 md:text-2xl">{t("eventsTitle")}</h2>
                <Link
                  href="/agenda"
                  className="inline-flex items-center gap-1 text-xs font-medium text-royal-600 hover:text-royal-700"
                >
                  ...
                  return (
                    <Reveal key={item.id} index={index}>
                      <Link
                        ...
                        className="group flex w-full flex-1 items-start gap-4 rounded-xl border border-slate-100 bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                      >
                        {startsAt ? (
                          <time className="flex shrink-0 flex-col items-center justify-center rounded-lg border border-royal-100 bg-gradient-to-b from-royal-50 to-white px-3 py-2 text-center">
