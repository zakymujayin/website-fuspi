...
export type TaxonomyKindFilter = "ALL" | TaxonomyKind;

/** Mirrors the frozen `CmsSearchSchema` bound so the control cannot submit an over-long term. */
export const TAXONOMY_SEARCH_MAX_LENGTH = 120;

/**
 * Builds an admin taksonomi-list link preserving the active filter; `@/i18n/navigation`'s `Link`
 * adds the locale. Only non-canonical members are serialized — in the fixed order kind, search,
 * pageSize, page — so page size 10, an empty search, and page 1 leave a bare URL.
 */
export function buildTaxonomyHref(parts: {
  kind?: TaxonomyKindFilter;
  search?: string;
  page?: number;
  pageSize?: 10 | 20 | 50;
}): string {
  const params = new URLSearchParams();
  if (parts.kind && parts.kind !== "ALL") params.set("kind", parts.kind);
  ...
  return query ? `/admin/taksonomi?${query}` : "/admin/taksonomi";
}
