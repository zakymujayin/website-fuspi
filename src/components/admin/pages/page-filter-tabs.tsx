...
export function AdminPageFilterTabs({ active, ariaLabel, labels, search, sort }: AdminPageFilterTabsProps) {
  return (
    <nav aria-label={ariaLabel} className="flex flex-wrap gap-2">
      {FILTERS.map((filter) => {
        ...
        return (
          <Link
            key={filter}
            href={buildAdminPageHref({ status: filter, search, sort, page: 1 })}
