...
export function AdminPostFilterTabs({ active, ariaLabel, labels, basePath, search, pageSize }: AdminPostFilterTabsProps) {
  return (
    <nav aria-label={ariaLabel} className="flex flex-wrap gap-2">
      {FILTERS.map((filter) => {
        ...
        return (
          <Link
            key={filter}
            href={buildAdminPostHref(filter, 1, basePath, { search, pageSize })}
