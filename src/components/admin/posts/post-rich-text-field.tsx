...
/**
 * Rich-text authoring field. Its toolbar exposes only the formats the server sanitizer keeps
 * (`src/lib/security/sanitize.ts` ALLOWED_TAGS) — bold, italic, H2/H3, lists, blockquote, code — so
 * authors never lose formatting silently on save. Output is HTML; the server sanitizes on write, so
 * this control is UX, not a security boundary.
 */
export function RichTextField({ value, onChange, ariaLabel, dir }: RichTextFieldProps) {
  const t = useTranslations("AdminPostRichText");
  const editor = useEditor({
    // Headings limited to the sanitizer's allowed levels (H2–H4; toolbar exposes H2/H3).
